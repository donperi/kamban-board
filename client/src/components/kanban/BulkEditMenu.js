...
const BulkEditMenu = React.forwardRef(
  ({ selectedTasks, styles, className }, ref) => {
    ...
    const handleUserCreate = async (input) => {
      try {
        ...
        return Promise.reject(
          new Error("An error occurred creating the user.")
        );
      }
    };
          ...
          render={(props) => (
            <Form onSubmit={props.handleSubmit}>
              ...
              <Form.Group className="text-right">
                <Button variant="info" className="mr-2" type="submit">
                  Update Tasks
                </Button>
              </Form.Group>
            </Form>
